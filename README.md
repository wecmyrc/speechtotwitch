...
Используем `key.txt`? Если заполнишь `key.txt`, то не придется каждый раз вручную писать ключ.

<kbd>Enter</kbd> - используем
<kbd>n</kbd> - хочу вручную

Если любишь писать, то имей в виду, что `Username` - имя пользователя, `OAuth` - ключ для этого пользователя.
